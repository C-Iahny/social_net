...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
	...
	async def new_post_notification(self, event):
		"""
		Called by the channel layer when a friend publishes a new post.
		Forwards the data to the WebSocket client.
		"""
		await self.send_json(
			{
				"general_msg_type": GENERAL_MSG_TYPE_NEW_POST_NOTIFICATION,
				"from_username": event["from_username"],
				"from_image":    event["from_image"],
	...
	async def post_action_notification(self, event):
		...
		# 2. Mettre à jour le badge immédiatement (type 4) — sans attendre le poll
		try:
			user = self.scope["user"]
			if user.is_authenticated:
				count_payload = await get_unread_general_notification_count(user)
				if count_payload:
					import json as _json
					parsed = _json.loads(count_payload)
					await self.send_json({
						"general_msg_type": GENERAL_MSG_TYPE_GET_UNREAD_NOTIFICATIONS_COUNT,
...
def refresh_general_notifications(user, oldest_timestamp, newest_timestamp):
	...
	if user.is_authenticated:
		...
		post_ct = ContentType.objects.get_for_model(PostModel)
		live_ct = ContentType.objects.get_for_model(LiveRoom)
		notifications = Notification.objects.filter(target=user, content_type__in=[friend_request_ct, friend_list_ct, post_ct, live_ct], timestamp__gte=oldest_ts, timestamp__lte=newest_ts).order_by('-timestamp')
...
def get_new_general_notifications(user, newest_timestamp):
	...
	if user.is_authenticated:
		...
		post_ct = ContentType.objects.get_for_model(PostModel)
		live_ct = ContentType.objects.get_for_model(LiveRoom)
		notifications = Notification.objects.filter(target=user, content_type__in=[friend_request_ct, friend_list_ct, post_ct, live_ct], timestamp__gt=timestamp, read=False).order_by('-timestamp')
		s = LazyNotificationEncoder()
		payload['notifications'] = s.serialize(notifications)
	else:
		raise ClientError("AUTH_ERROR", "User must be authenticated to get notifications.")
...
def get_unread_general_notification_count(user):
	...
	if user.is_authenticated:
		...
		unread_count = Notification.objects.filter(
			target=user,
			content_type__in=[friend_request_ct, friend_list_ct, post_ct, live_ct],
			read=False,
		).count()
